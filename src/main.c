...
void cargarPrimeraFila(int N, bool* continuar, FILE* entrada, unsigned char** matriz) {
	int i = 0;
	int aux;
...
unsigned char proximo(unsigned char *a, unsigned int i, unsigned int j,
	...
	// TODO: falta implementar y pasar a assembler
...
void calcularFilas(bool continuar, int N, int regla, unsigned char** matriz) {
	int i;
	int j;
	if (continuar == true) {
		for (i = 0; i < N - 1; i++) {
...
FILE* escribirImagen(bool continuar, int* N, unsigned char** matriz) {
	int i;
	int j;
	FILE* salida;
...
void liberarMemoriaYCerrarArchivos(int N, unsigned char** matriz, FILE* entrada, FILE* salida) {
	int i;
	for (i = 0; i < N; i++)
		free(matriz[i]);
	free(matriz);
...
int main(int argc, char *argv[]) {
	...
	unsigned char** matriz = crearMatriz(N);
